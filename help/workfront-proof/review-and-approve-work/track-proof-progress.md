...
若要在 [!DNL Workfront] 檢視校訂進度，您必須具有「計劃」、「工作」或「審閱」授權，而且是一位校訂使用者。 如果您不確定您的[!DNL Workfront]設定檔是否符合這些要求，請洽詢您組織的校訂系統管理員。

## 利用 [!UICONTROL SOCD] 指標和校訂狀態來追蹤校訂進度

使用「[!UICONTROL 文件]」清單中的 [!UICONTROL SOCD] 圖示，從高層次角度檢視校訂工作在審閱與核准流程中的進度。 這些圖示表示對校訂採取的特定動作。
...
比如，若有三位審閱者/核准者，而其中只有兩位看過校訂內容並留下註解，則 [!UICONTROL SOCD] 圖示會顯示校訂已傳送 ([!UICONTROL S]) 和已開啟 ([!UICONTROL O])，但是並未留下註解 ([!UICONTROL C])。
...
開啟校訂工作流程視窗，查看被指派為「[!UICONTROL 審閱者與核准者]」或「[!UICONTROL 核准者]」校訂角色的收件者之校訂狀態。

## [!DNL Workfront] 的報告

您還可以利用 [!DNL Workfront's] 報告功能，追蹤校訂在審閱與核准流程中的進度。
