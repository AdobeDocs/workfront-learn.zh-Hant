...
根據所需的特定資料而定，計算欄位可以是使用一或兩個運算式的簡單欄位，或是使用數個嵌入運算式的複雜欄位。 請記住，Workfront 的計算欄位只能使用已儲存或提取到系統中的資料。

## 文字運算式

文字運算式會搜尋、拆解分析和結合在 [!DNL Workfront] 中找到的資訊，建立更具有意義的資料，或是對於貴組織所執行的工作提供更深入的洞察。
