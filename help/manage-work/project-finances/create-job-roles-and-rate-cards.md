---
title: 建立職位角色和費率卡
description: 瞭解如何建立工作角色和費率卡，以及如何從專案附加和移除費率卡。
activity: use
team: Technical Marketing
feature: Work Management
type: Tutorial
role: User
level: Intermediate
jira: KT-20259
last-substantial-update: '2026-05-08T00:00:00.000Z'
recommendations: noDisplay,catalog
doc-type: video
product_v2:
  - id: c4a86a5d-6562-4fc6-aa00-bfa25833aed9
feature_v2:
  - id: d968a1bc-9a90-4926-a531-bcf272c32aad
  - id: a0dacc9f-0e23-495b-8e9f-a77c2e60b40c
subfeature_v2:
  - id: f0dd7b45-76b5-49d4-afe3-39f436b6fbd3
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2:
  - id: b5a62a22-46f7-4f0d-b151-3fc640bef588
autotag-review: '2026-05-06T14:51:17.520Z'
source-git-commit: ee03983e89e8c99e2fbdc12b108f3fb33d6d8e72
workflow-type: tm+mt
source-wordcount: 205
ht-degree: 13%

---

# 建立職位角色和費率卡

影片說明如何建立工作角色和費率卡，以及如何從專案附加和移除費率卡。


>[!VIDEO](https://video.tv.adobe.com/v/3486517/?quality=12&learn=on&enablevpops=1)

## 重點提要

* **工作角色結構專案任務：**&#x200B;工作角色代表任務所需的技能，可以根據使用者的能力和可用性指派給使用者。
* **費率卡定義收費率：**&#x200B;費率卡會列出工作角色的收費率，並做為專案收費的客戶合約。
* **彈性費率管理：**&#x200B;職務角色和費率卡都允許設定有效日期的成本和帳單費率，支援以時間為基礎的定價變更。
* **費率卡覆寫：**&#x200B;將費率卡附加至專案時，其費率會覆寫工作角色費率，但專案特定的覆寫不受影響。
* **簡單的附件和更新：**&#x200B;可從專案中附加或移除費率卡，而且可將更新推送至現有專案以確保帳單準確。

## 有關此主題的推薦教學課程

* [瞭解績效量度](/help/manage-work/project-finances/understand-performance-metrics.md)
* [設定任務的預設收入與成本](/help/manage-work/project-finances/set-up-task-revenue-and-cost-defaults.md)
* [設定匯率](/help/manage-work/project-finances/set-up-exchange-rates.md)
* [設定費用類型](/help/manage-work/project-finances/set-up-expense-types.md)
* [瞭解財務資訊存取](/help/manage-work/project-finances/understand-financial-access.md)
