...
為了將請求與具有「目前」狀態的實際工作專案區分開來，請建立僅用於名為「請求佇列」或「作業」的請求佇列的狀態。 然後，您可以使用此狀態來協助在撰寫報告時排除或包含請求佇列專案。
...
在大多數情況下，請求佇列只需要與特定一組人員共用，例如團隊、供應商、客戶等。當請求者只在請求佇列清單中看到他們需要的內容時，可讓您輕鬆找到並導覽。
...
這個做法也能確保，當流量管理員或負責審閱或指派收到的請求之其他人員無暇處理時，自然會有其他後備人員接手而且擁有該請求資訊的存取權。
