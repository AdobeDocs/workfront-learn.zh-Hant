...
* 停用Workfront設定中的「當對校訂進行評論時從Workfront傳送電子郵件」設定。
...
預設為24小時時鐘格式，這可能會讓不熟悉的人感到困惑。 若要變更格式，請前往Workfront主要功能表>校樣>帳戶設定>使用者。 連按兩下某個使用者以選取一個使用者，然後編輯「個人設定」區段中的「日期格式」欄位。 您必須一次選取一個使用者才能變更他們。
