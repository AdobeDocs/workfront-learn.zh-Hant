...
* 指派「審閱」和「請求」授權時，一般會預設為「審閱」，因為它授予使用者更多使用 Adobe Workfront 的權限。
...
雖然「審閱」和「請求」授權可以指派給 Workfront 中無限多位使用者，但「請求」授權僅有提出請求和更新請求的權限而已。 「審閱」授權對專案和任務擁有比「請求」授權更多的存取權，也可以檢視專案組合和方案、編輯文件以及存取資源管理工具。
...
例如，您可能會與Workfront中的數家廠商合作，以檢查進度、提供核准等。如果「共用系統範圍」核取方塊是一個選項，可選取或設定為預設值，讓所有廠商都可取得資訊。
...
請提供詳細的說明，列出每個物件類型的設定。 這樣可以協助現在和未來的系統管理員清楚知道每個存取層級有什麼能力，而不必深入研究存取層級來審閱相關設定。
...
存取層級可確保在和使用者共用Workfront物件時，使用者擁有編輯、刪除物件等所需的許可權。您可以將存取層級設為較一般化，因為個別專案上的共用可設為較具體。
